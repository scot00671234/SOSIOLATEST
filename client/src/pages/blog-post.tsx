...
Traditional discussion boards often required complex registration processes and had steep learning curves that discouraged casual participation. Modern platforms like Sosiol maintain the organizational benefits of classic forums while removing barriers to entry. The familiar Reddit-style interface provides immediate usability for users accustomed to nested comments and voting systems.
...
export default function BlogPostPage() {
  ...
  return (
    <>
      <Helmet>
        <title>{post.title} | Sosiol Blog</title>
        <meta name="description" content={post.excerpt} />
        <meta name="keywords" content={`${post.category.toLowerCase()}, reddit alternative, free speech social media, community platform, ${post.slug.replace(/-/g, ', ')}`} />
        <meta property="og:title" content={post.title} />
        <meta property="og:description" content={post.excerpt} />
        <meta property="og:type" content="article" />
        <meta property="og:url" content={`https://sosiol.com/blog/${post.slug}`} />
        <meta property="article:published_time" content={post.publishDate} />
        <meta property="article:section" content={post.category} />
        <link rel="canonical" href={`https://sosiol.com/blog/${post.slug}`} />
      </Helmet>
          ...
          <article>
            <header className="mb-8">
              <div className="flex items-center gap-4 mb-4">
                ...
                <div className="flex items-center text-sm text-muted-foreground" data-testid="text-publish-date">
                  <Calendar className="h-4 w-4 mr-1" />
                  {formatDate(post.publishDate)}
                </div>
                <div className="flex items-center text-sm text-muted-foreground" data-testid="text-read-time">
                  <Clock className="h-4 w-4 mr-1" />
                  {post.readTime}
                </div>
              </div>
              <h1 className="text-4xl font-bold text-foreground mb-4" data-testid="text-post-title">
                {post.title}
              </h1>
            ...
            <div className="prose prose-lg max-w-none dark:prose-invert" data-testid="text-post-content">
              {post.content.split('\n\n').map((paragraph, index) => (
                paragraph.trim() && (
